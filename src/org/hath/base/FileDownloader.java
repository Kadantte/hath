		...
		synchronized (downloadLock) {
			...
			while (retries-- > 0) {
				...
				try {
					...
					connection.setReadTimeout(timeout); // this doesn't always seem to work however, so we'll do it
					...
					if (contentLength < 0) {
						// H@H note: since we control all systems in this case, we'll demand that clients and servers
						// always send the Content-Length
						...
						// malformed oversized reply, so we'll limit the download size to the H@H max (10 MB). the
						// server will never send anything this large as a response either.
